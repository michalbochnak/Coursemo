...
# on waitlist: {8}",
    ...
    private bool StudentEnrolled(int sid, int cid)
    {
      ...
      MessageBox.Show("StudentEnrolled(): " + sid + ", " + cid + ", " + enrolled);
      ...
      try
      {
        ...
        else
        {
          ...
          if (result == -1)
            ...
            InfoLabel.Text = "Dropped, waitlist empty";
          else if (result == 1)
    ...
    private int Swap(int s1, int s2, int c1, int c2)
    {
      try
      {
          ...
          txOptions))
        {
          ...
          //
          // FIXME: StudentEnrolled(s2, c2) returns wrong value (0)
          //
    ...
    {
      var query = from r in db.Registrations select r;
      db.Registrations.DeleteAllOnSubmit(query);
      query = from w in db.Waitlists select w;
      db.Waitlists.DeleteAllOnSubmit();
