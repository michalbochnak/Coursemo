...
namespace Coursemo
{
  public partial class Form1 : Form
  {
    ...
    private void SwapButton_Click(object sender, EventArgs e)
    {
      ...
      int cIdx = this.CoursesListBox.SelectedIndex;
      int s2 = this.EnrolledInListBox.SelectedIndex;
      ...
      {
        this.InfoLabel.Text = "Please select student from course enrollment list.";
        return;
      }
      // swap
      else
      {
        int result = Swap(_students[s1].SID, _students[s2].SID, _courses[cIdx].CID);
      }
    }


    private int Swap(int s1, int s2, int cid)
    {
      try
      {
        var txOptions = new TransactionOptions();
        ...
        using (var transaction = new TransactionScope(TransactionScopeOption.Required,
          txOptions))
        {
          // both students must be enrolled, drop course and enroll first student on waitlist
          if (StudentEnrolled(s1, cid) && StudentEnrolled(s2, cid))
          {
            db.UnregisterStudent(s2, cid);
              db.RegisterStudent(s1, cid);
              db.UnwaitlistStudent(s1, cid);
              db.SubmitChanges();
              transaction.Complete();
              return 1;
            }
