  ...
  public partial class Form1 : Form
  {
    ...
    private void StudentsListBox_SelectedIndexChanged(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        MessageBox.Show("Got: " + query.Count());
    ...
    private bool Enroll(int sid, int cid)
    {
      ...
      try
      {
          ...
          txOptions))
        {
          ...
          {
            db.Waitlists.InsertOnSubmit(new Waitlist
            {
              SID = sid, CID = cid
            });
          ...
          {
            db.Registrations.InsertOnSubmit(new Registration
            {
              SID = (from s in db.Students where s.SID == sid select s.SID).Single(),
              CID = (from c in db.Courses where c.CID == cid select c.CID).Single()
            });
            transaction.Complete();
            db.SubmitChanges();
            MessageBox.Show("Enrolled s: " + sid + "c: " + cid);
            return true;
          }
        }
      }
